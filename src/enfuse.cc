...
enum AllPossibleOptions {
    ...
    BlockSizeOption, BlendColorspaceOption, CIECAM02Option, NoCIECAM02Option, FallbackProfileOption,
                    ...
                    const std::string& an_option_name)
{
    if (an_option_argument != NULL && *an_option_argument != 0) {
        boost::char_separator<char> sep(PATH_OPTION_DELIMITERS, "", boost::keep_empty_tokens);
...
process_options(int argc, char** argv)
{
    ...
    while (true) {
        ...
        case EntropyCutoffId: {
            ...
            if (token != tok.end()) {
                std::cerr << command << ": warning: ignoring trailing garbage \""
                          << *token << "\" in argument to \"--entropy-cutoff\"" << endl;
            }
        ...
        case ExposureCutoffId: {
            ...
            if (token != tok.end()) {
                std::cerr << command << ": warning: ignoring trailing garbage \""
                          << *token << "\" in argument to \"--exposure-cutoff\"" << endl;
            }
        ...
        case CiecamId:
            std::cerr <<
                command << ": info: prefer option \"--blend-colorspace\" to \"--ciecam\"" << std::endl;
            BlendColorspace = CIECAM;
        ...
        case NoCiecamId:
            std::cerr <<
                command << ": info: prefer option \"--blend-colorspace\" to \"--no-ciecam\"" << std::endl;
            BlendColorspace = IdentitySpace;
