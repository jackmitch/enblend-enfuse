...
\item
  ...
  \begin{restrictedmaterial}{OS X}
    If \metavar{SHARED-OBJECT} does not contain a slash (\sample{/}), the dynamic loader 
    searches the following until it finds a compatible Mach-O file: \envvar{LD\_LIBRARY\_PATH}, 
    \envvar{DYLD\_LIBRARY\_PATH}, current working directory, \envvar{DYLD\_FALLBACK\_LIBRARY\_PATH}.
...
\item
  \begin{sloppypar}
    ...
    package -- in directory \filename{\val*{val:DOCDIR}/\feasiblebreak examples/\feasiblebreak
      enfuse}.
  \end{sloppypar}
...
directory~\filename{\val*{val:DOCDIR}/\feasiblebreak examples/\feasiblebreak enfuse} along with
a \acronym{GNU}-Makefile called \filename{Makefile.userweight}.
...
\begin{restrictedmaterial}{OS X}
  \noindent On OS X the creation of shared objects -- or loadable modules --
  has been tested with the \acronym{Clang} C language frontend for \acronym{LLVM},
  ...
  \begin{codelist}
  \item[\option{-bundle}]\itemend
    Instruct the compiler's code-generator to produce a loadable module.
  \end{codelist}
