...
option~\option{--exposure-weight-function} and load the exposure weight function identifed by
\metavar{SYMBOL} from \metavar{SHARED-OBJECT}:
...
\item
  ...
  \App{}, \metavar{SHARED-OBJECT} may or may not require a path.
  ...
  \begin{restrictedmaterial}{Windows}
    If \metavar{SHARED-OBJECT} specifies an absolute filename, this file is used.
    Otherwise it searches in the following directories in given order:
    \begin{enumerate}  
      \item The directory from which \app{} is loaded.
    ...
    \end{enumerate}
    For more details consult the manual page of LoadLibrary.
  \end{restrictedmaterial}
