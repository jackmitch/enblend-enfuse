...
\item[--anneal=\metavar{TAU}\optional{:\metavar{DELTA-E-MAX}\optional{:\metavar{DELTA-E-MIN}\optional{:\metavar{K-MAX}}}}]\itemend
  Set the parameters\genidx{optimize!anneal parameters}\genidx{anneal
    parameters} of the Simulated Annealing\genidx{simulated annealing
    optimizer}\genidx{optimizer!simulated annealing} optimizer.  See
  Table~\fullref{tab:optimizer-strategies}.

  \begin{codelist}
  \item[\metavar{TAU}] \metavar{TAU} is the temperature reduction
    factor in the Simulated Annealing; it also can be thought of as
    ``cooling factor''.  The closer \metavar{TAU} is to one, the more
    accurate the annealing run will be, and the longer it will take.

    Append a percent sign (\sample{\%}) to specify \metavar{TAU} as a
    percentage.

    Valid range: $\val{val:minimum-anneal-tau} < \metavar{TAU} <
    \val{val:maximum-anneal-tau}$.

    The default is \val{val:default-anneal-tau}; values around 0.95 are
    reasonable.  Usually, slower cooling results in more converged
    points.

  \item[\metavar{DELTA-E-MAX}, \metavar{DELTA-E-MIN}]\itemend
    \metavar{DELTA-E-MAX} and \metavar{DELTA-E-MIN} are the maximum
    ...
    The defaults are: \metavar{DELTA-E-MAX}:
    \val{val:default-anneal-deltae-max} and \metavar{DELTA-E-MIN}:
    \val{val:default-anneal-deltae-min}.

  \item[\metavar{K-MAX}]\itemend \metavar{K-MAX} is the maximum number
    ...
    The default is \val{val:default-anneal-kmax}.  Values around 100
    seem reasonable.
  \end{codelist}
...
\item[--dijkstra=\metavar{RADIUS}]\itemend Set the search
  \metavar{RADIUS} of the \propername{Dijkstra} Shortest Path
  algorithm used in \propername{Dijkstra} Optimization (see
  Table~\fullref{tab:optimizer-strategies}).
...
\item[\itempar{--image-difference=\metavar{ALGORITHM}\optional{:\feasiblebreak
  ...
  \App{} calculates the difference\genidx{image difference}
  \genidx{difference image} of a pair of overlapping color images when
  it generates the primary seam with a Graph-Cut and also before it
  optimizes the seams.  It employs a user-selectable
  \metavar{ALGORITHM} that is controlled by the
  \begin{itemize}
  \item
    weights for luminance differences, \metavar{LUMINANCE-WEIGHT},
    $w_{\mathrm{luma}}$\genidx{luminance
      weight}\genidx{weight!luminance} and
  \item
    color differences, \metavar{CHROMINANCE-WEIGHT},
    $w_{\mathrm{chroma}}$.\genidx{chrominance
      weight}\genidx{weight!chrominance}
  \end{itemize}
  ...
  \item[\itempar{maximum-hue-luminance \\ maximum-hue-lum
    ...
    the difference~$d$ as the maximum\genidx{maximum hue-luminance}
    \genidx{hue-luminance maximum} \genidx{luminance-hue maximum} of
  ...
  \item[\itempar{delta-e \\ de}\genidx{delta-E}]\itemend Calulate the
    difference~$d$ as the \propername{Euclidean} distance of the
    pixels in \acronym{L*a*b*}\genidx{colorspace!\acronym{L*a*b*}}
    space:
    ...
    \metavar{DELTA-E-MAX} and \metavar{DELTA-E-MIN} of
    option~\option{--anneal}.
  \end{codelist}

  \begin{sloppypar}
    Both \metavar{LUMINANCE\hyp{}WEIGHT} and
    \metavar{CHROMINANCE\hyp{}WEIGHT} are non-negative.  \App{}
    automatically normalizes the sum of
    \metavar{LUMINANCE\hyp{}WEIGHT} and
  ...
  The default \metavar{LUMINANCE\hyp{}WEIGHT} is
  \val{val:default-luminance-difference-weight} and the default
  \metavar{CHROMINANCE\hyp{}WEIGHT} is
  ...
  \metavar{ALGORITHM}, therefore they should only be compared for
  identical \metavar{ALGORITHM}s.  The average difference is a rough
  measure of quality\genidx{match quality}\genidx{quality!match} with
...
\item[--mask-vectorize=\metavar{DISTANCE}]\itemend Set the mask
  vectorization \metavar{DISTANCE}\genidx{mask!vectorization distance}
  that \App{} uses to partition each seam.  Thus, break down the seam
  to segments of length \metavar{DISTANCE} each.

  If \App{} uses a coarse mask (\option{--coarse-mask}) or \App{}
  optimizes (\option{--optimize}) a mask it vectorizes the initial
  seam line before performing further operations.  See
  Table~\ref{tab:mask-generation} for the precise conditions.
  \metavar{DISTANCE} tells \App{} how long to make each of the line
  segments called vectors here.

  The unit of \metavar{DISTANCE} is pixels unless it is a percentage
  ...
  \val{val:default-coarseness-factor}~pixels in the input image.

  Append a percentage sign (\sample{\%}) to \metavar{DISTANCE} to
  ...
  \metavar{DISTANCE} values lead to shorter, straighter, less wiggly,
  ...
  avoiding them.  Small \metavar{DISTANCE} values give the optimizers
  more possibilities to run the seam around high mismatch areas.

  What should \emph{never} happen though, are loops\genidx{loops in
    seam line}\genidx{seam line!loops} or cusps in the seam line.
  Counter loops and cusps with higher weights of
  \metavar{DISTANCE-WEIGHT} (option~\option{--optimizer-weights}),
  larger vectorization \metavar{DISTANCE}s, and \metavar{TAU}s
  (option~\option{--anneal}) that are closer to one.  Use
  option~\option{--visualize} to check the results.
  ...
  \val{val:minimum-vectorize-distance} even if it has been given as a
  percentage.  The user will be warned in such cases.

  Defaults: \val{val:coarse-mask-vectorize-distance}~pixels for
  coarse masks and \val{val:fine-mask-vectorize-distance}~pixels
  for fine masks.
...
\item[--optimizer-weights=\metavar{DISTANCE-WEIGHT}\optional{:\metavar{MISMATCH-WEIGHT}}]\itemend
  Set the weights\genidx{optimizer weights}\genidx{weight!optimizer}
  of the seam-line optimizer.\genidx{seam optimization} If omitted,
  \metavar{MISMATCH-WEIGHT} defaults to 1.
  ...
  \item
    ...
    option~\option{--no-optimize}).
  \item
    The total ``mismatch'' accumulated along it.
  \end{itemize}

  \metavar{DISTANCE-WEIGHT} and \metavar{MISMATCH\hyp{}WEIGHT} define
  ...
  A large \metavar{DISTANCE-WEIGHT} pulls the optimized seam line
  closer to the initial postion.  A large
  \metavar{MISMATCH\hyp{}WEIGHT} makes the seam line go on detours to
  find a path along which the mismatch between the images is small.
  If the optimized seam line shows cusps or loops (see
  option~\option{--visualize}), reduce \metavar{MISMATCH\hyp{}WEIGHT}
  or increase \metavar{DISTANCE\hyp{}WEIGHT}.
...
\item[--primary-seam-generator=\metavar{ALGORITHM}]\itemend
  ...
  generating\genidx{generator!seam} the general\genidx{primary seam
    generator}\genidx{seam!primary generator} seam route.

  This is the \metavar{ALGORITHM} that produces an initial seam line,
  which serves as the basis for later, optional optimizations.
  Nearest Feature Transform (\acronym{NFT}) is the only algorithm up
  to and including \App{} version~4.0.  Version~4.1 added a Graph-Cut
  (\acronym{GC}) algorithm, which is the default for version~4.2 and
  later.
  ...
  See Chapter~\fullref{sec:seam-generators} for details on \App's
