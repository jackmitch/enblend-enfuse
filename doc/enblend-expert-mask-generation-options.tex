...
\item[\itempar{--image-difference=\metavar{ALGORITHM}\optional{:\feasiblebreak
  ...
  \item[\itempar{delta-e \\ de}]\itemend
    Calulate the difference~$d$ as the \propername{Euclidean} distance of the pixels in
    \acronym{L*a*b*} space:
    ...
    \metavar{DELTA-E-MIN} of option~\option{--anneal}.
  \end{codelist}
...
\item[--mask-vectorize=\metavar{DISTANCE}]\itemend
  ...
  If \App{} uses a coarse mask (\option{--coarse-mask}) or \App{} optimizes
  (\option{--optimize}) a mask it vectorizes the initial seam line before performing further
  operations.  See \tableName~\ref{tab:mask-generation} for the precise conditions.
  \metavar{DISTANCE} tells \App{} how long to make each of the line segments called vectors
  here.
  ...
  (option~\option{--optimizer-weights}), larger vectorization \metavar{DISTANCE}s, and
  \metavar{TAU}s (option~\option{--anneal}) that are closer to one.  Use
...
\item[--optimizer-weights=\metavar{DISTANCE-WEIGHT}\optional{:\metavar{MISMATCH-WEIGHT}}]\itemend
  ...
  \item
    ...
    \acronym{NFT} (see option~\option{--no-optimize}).
  ...
  A large \metavar{DISTANCE-WEIGHT} pulls the optimized seam line closer to the initial postion.
  A large \metavar{MISMATCH\hyp{}WEIGHT} makes the seam line go on detours to find a path along
  which the mismatch between the images is small.  If the optimized seam line shows cusps or
  loops (see option~\option{--visualize}), reduce \metavar{MISMATCH\hyp{}WEIGHT} or increase
  \metavar{DISTANCE\hyp{}WEIGHT}.
