...
\genidx{profile!\acronym{ICC}}\uref{\wikipediaiccprofile}{\acronym{ICC}}-color profiles,
\gensee{ICC@\acronym{ICC} profile}{profile, \acronym{ICC}} blend color spaces in
\application{Enblend} or \application{Enfuse}.

Here, we collectively speak of \genidx{pixels!blending}\gensee{blending pixels}{pixels,
  blending}blending and do not distinguish \genidx{pixels!fusing}\gensee{fusing pixels}{pixels,
  fusing}fusing, for the basic operations are the same.  Furthermore, we assume the
...
their luminances~$L_1$, $L_2$ and their \genidx{weighting factor}weighting
factor~$0 \leq w \leq 1$, what \genidx{luminance}luminance~$L$ is their ``weighted average''?
This is the heart of \application{Enblend's} and \application{Enfuse's} pyramidal blending
...
\begin{geeknote}
  \noindent Note that \application{Enblend} and \application{Enfuse} face different obstacles in
...
value~$L_{\mathrm{max}}$, thereby defining a \genidx{luminance interval}``luminance
interval''. We can always map this interval to $(0, 1)$ by dividing the luminances by the
maximum, which is why we call the latter \genidx{luminance
  interval!normalized}\emph{``normalized luminance interval''}:
...
Sometimes images are gamma-encoded with \genidx{luminance interval!normalized}exponent~$\gamma$
and the blended luminance becomes
...
\genidx{transform!floating-point images}Floating-point images
(\genidx{image!\acronym{EXR}}\gensee{EXR@\acronym{EXR} image}{image,
  \acronym{EXR}}\acronym{EXR}, \genidx{image!floating-point
  \acronym{TIFF}}\gensee{floating-point \acronym{TIFF} image}{image, floating-point
  \acronym{TIFF}}floating-point \acronym{TIFF}, or \genidx{image!floating-point
  \acronym{VIFF}}\gensee{floating-point \acronym{VIFF} image}{image, floating-point
  \acronym{VIFF}}\gensee{VIFF@\acronym{VIFF} floating-point image}{image, floating-point
  \acronym{VIFF}}\acronym{VIFF}) get a special treatment.  Their values~$L$ are first converted
by the \genidx{transform!log}\gensee{log-transform}{transform, log}\LogTransform-transform.
...
\begin{figure}[htbp]
  ...
  \caption[\LogTransform-transform]{\label{fig:log-transform}\genidx{transform!log}%
    Forward \LogTransform-transform shown for the range of $-20 \leq L \leq 100$.  The domain of
    $\LogTransform(L)$ is the whole real axis.}
\end{figure}
...
In the current version of \application{Enblend} and \application{Enfuse} it is \emph{strongly
  recommended} to use blending inside the \acronym{RGB}-cube whenever the input data is in
...
\application{Enblend} and \application{Enfuse} decode and encode images automatically respecting
the gamma curves.  Moreover color profiles define what is the darkest representable black, so
called \genidx{black-point}black-point
\[
  L = 0
  \quad \mbox{and}\quad
  \transpose{(r, g, b)} = \transpose{(0, 0, 0)}
\]
and analogously what is the purest and brightest white, the \genidx{white-point}white-point
\[
  L = 1
  \quad \mbox{and}\quad
...
\noindent By default, \application{Enblend} and \application{Enfuse} expect that either
...
Even \genidx{image!black-and-white}\gensee{black-and-white image}{image,
  black-and-white}black-and-white images benefit from having attached appropriate profiles!

In Case~\ref{enum:no-profile}.\ the applications blend grayscale images in the normalized
luminance interval and color images inside the
...
\acronym{sRGB}-cube.  To override the default \acronym{sRGB}-profile select the desired profile
with option~\flexipageref{\option{--fallback-profile}}{opt:fallback-profile}.
...
\application{Enblend} and \application{Enfuse} offer to work inside the
\acronym{RGB}-cube~\eqnref{equ:rgb-cube} or in several perceptually uniform color spaces.  To
override the default select a particular blending color space with
option~\flexipageref{\option{--blend-colorspace}}{opt:blend-colorspace}.  Here are the four
...
\item[Identity Space / \acronym{RGB}-Color Cube]\itemend
  ...
  for grayscale images and inside the \genidx{color cube!\acronym{RGB}}\gensee{RGB@\acronym{RGB}
    color cube}{color cube, \acronym{RGB}}\acronym{RGB}-color cube as given in
...
\item[\urlmark{\wikipedialabcolorspace}{\acronym{L*a*b*}}]\itemend\urltext
  ...
  \genidx{colorspace!\acronym{L*a*b*}}\gensee{L*a*b*@\acronym{L*a*b*} colorspace}{colorspace,
    \acronym{L*a*b*}}\acronym{L*a*b*} color space encompasses all perceivable colors.  It is
  ...
  \genidx{white-point!\acronym{D50}}\gensee{D50@\acronym{D50} white-point}{white-point,
    \acronym{D50}}\acronym{D50} white-point (see,
...
\item[\urlmark{\wikipediacieluvcolorspace}{\acronym{CIEL*u*v*}}]\itemend\urltext
  ...
  The \genidx{colorspace!\acronym{CIEL*u*v*}}\gensee{CIEL*u*v*@\acronym{CIEL*u*v*}
    colorspace}{colorspace, \acronym{CIEL*u*v*}}\acronym{L*u*v*} tries to be percepually uniform
...
\item[\urlmark{\wikipediaciecam}{\acronym{CIECAM02}}]\itemend\urltext
  ...
  \genidx{colorspace!\acronym{CIECAM02}}\gensee{CIECAM02@\acronym{CIECAM02}
    colorspace}{colorspace, \acronym{CIECAM02}}\acronym{CIECAM02} color space and back use
  \genidx{rendering intent!perceptual}\gensee{perceptual rendering intent}{rendering
    intent}perceptual rendering intent, the
  \genidx{white-point!\acronym{D50}}\gensee{D50@\acronym{D50} white-point}{white-point,
    \acronym{D50}}\acronym{D50} white point (see,
  e.g.\ \uref{\wikipediastandardilluminant}{Standard illuminant}), 500\dmn{lumen} surrounding
  light (``average'' in \acronym{CIECAM02} parlance), and assume complete adaption.
