...
\item[--entropy-cutoff=\metavar{LOWER-CUTOFF}\optional{:\metavar{UPPER-CUTOFF}}]\itemend
  Define the \metavar{LOWER-CUTOFF} value below which pixels are treated as pure black when
  calculating the local entropy.  Optionally also define the \metavar{UPPER-CUTOFF} value above
  which pixels are treated as pure white.
  ...
  \figureName~\fullref{fig:entropy-cutoff} shows an example.
  ...
  \begin{figure}[htbp]
    ...
    \caption[Entropy cutoff function]{\label{fig:entropy-cutoff}%
      Linear lightness~\metavar{Y} in comparison with an entropy-cutoff function for
      $\metavar{LOWER-CUTOFF} = 5\%$ and $\metavar{UPPER-CUTOFF} = 90\%$, which are rather
      extreme values.  Please note that we have shifted the original lightness curve up and the
      cut-off curve down by a very small \metavar{Offset} to emphasize that the proportional
      part of \metavar{Y} remains unaltered under the cut-off operation.}
  \end{figure}

  Note that a high \metavar{LOWER-CUTOFF} value lightens the resulting image, as dark and
  presumably noisy pixels are averaged with \emph{equal} weights.  With $\metavar{LOWER-CUTOFF}
  ...
  Analogously, a low \metavar{UPPER-CUTOFF} darkens the output image.
...
\item[\itempar{--exposure-cutoff=\metavar{LOWER-CUTOFF}\optional{:\feasiblebreak
  ...
  Set the \metavar{LOWER-CUTOFF} and the \metavar{UPPER-CUTOFF}.  Pixels below the lower or
  above the upper cutoff get a weight of exactly zero irrespective of the active exposure-weight
  function.
  ...
  \command{convert}\prgidx{convert} (see \appendixName~\fullref{sec:helpful-programs}) prior to
  fusing with the commands demonstrated in \exampleName~\ref{ex:imagemagick-convert-cutoff}.
  ...
  \begin{exemplar}[htbp]
    ...
    \caption[\application{ImageMagick} for exposure cutoff]{\label{ex:imagemagick-convert-cutoff}%
      Using \application{ImageMagick} for exposure cutoff operations.  The first example only
      applies a lower cutoff, wherease the second one applies both a lower and an upper cutoff
      to the images.}
  \end{exemplar}

  Transforming some or all input images as shown in the example gives the user more flexibility
  because the thresholds can be chosen for each image individually.
  ...
  \begin{figure}
    ...
    \caption[Exposure cutoff function]{\label{fig:exposure-cutoff}%
      Exposure weight, a \propername{Gaussian} with the $\metavar{optimum} = 0.5$ and
      $\metavar{width} = 0.2$ submitted to an exposure-cutoff of $\metavar{LOWER-CUTOFF} = 5\%$
      and $\metavar{UPPER-CUTOFF} = 97\%$.}
  \end{figure}
