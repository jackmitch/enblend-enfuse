...
A binary mask\genidx{mask!binary}\gensee{binary mask}{mask, binary}
indicates for every pixel of an image if this pixel must be considered
in further processing, or ignored.  For a weight
mask,\genidx{mask!weight} the value of the mask determines how much
...
Each of the input files for \application{Enfuse} and
\application{Enblend} can contain its own mask.  Both applications
interpret them as binary masks no matter how many bits per image pixel
they contain.

Use ImageMagick's \command{identify}\prgidx{identify
  \textrm{(ImageMagick)}} (see Example~\ref{ex:using-identify}) or,
for \acronym{TIFF} files only, \command{tiff\/info}\prgidx{tiffinfo
  \textrm{(libtiff)}} (see Example~\ref{ex:using-tiffinfo}) to inquire
quickly whether a file contains a mask.
Appendix~\fullref{sec:helpful-programs} shows where to find these
programs on the web.
...
\begin{exemplar}[htbp]
  ...
  \caption[Using \command{identify}.]{Using \command{identify} to find
    out about the mask in \filename{image-0000.tif}.  \sample{Matte}
    indicates the existence of a mask.\label{ex:using-identify}}
\end{exemplar}
...
\begin{exemplar}[htbp]
  ...
  \caption[Using \command{tiff\/info}.]{Using \command{tiff\/info} to
    find out about the mask in \filename{image-0000.tif}.  Here the
    line \sample{Extra Samples} indicates one extra sample per pixel,
    which is interpreted as an unassociated alpha-channel;
    \application{Enblend} and \application{Enfuse} interpret this as
    mask.  The second hint \command{tiff\/info} gives is in
    \sample{Samples/Pixel}, where -- for a \acronym{RGB}-image -- the
    $4 = 3 + 1$ tells about the extra
    channel.\label{ex:using-tiffinfo}}
\end{exemplar}
...
\begin{description}
\item[Mask]\itemend A mask\genidx{mask} defines a selection of pixels.
  ...
  \uref{\gimpsavvy}{Gimp-Savy.}

\item[Alpha Channel]\itemend \genidx{channel!alpha} The alpha
  channel\genidx{alpha channel} stores the transpacency value for each
...
\item[Matte]\itemend The notion ``matte''\genidx{matte} as used by
  ImageMagick refers to an inverted alpha channel, more precisely: $1
  - \mbox{alpha}$.  See
  \uref{\imagemagickorgusagechannelstrans}{ImageMagick} for further
  explanations.
\end{description}

\application{Enblend} and \application{Enfuse} only consider pixels
that have an associated mask value other than zero.  If an input image
does not have an alpha channel, \application{Enblend} warns and
