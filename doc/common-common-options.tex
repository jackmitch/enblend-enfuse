...
\item[--compression=\metavar{COMPRESSION}]\itemend Write a compressed
  ...
  \item[\acronym{JPEG} format.]\itemend The compression either is a
    ...
    \item[\metavar{LEVEL}]\itemend Set \acronym{JPEG}
      quality~\metavar{LEVEL}, where \metavar{LEVEL} is an integer
    ...
    \item[jpeg-arith\optional{:\metavar{LEVEL}}]\itemend Switch on
      arithmetic \acronym{JPEG} compression.  With optional argument
      set the arithmetic compression~\metavar{LEVEL}, where
      \metavar{LEVEL} is an integer that ranges from 0--100.
    \end{codelist}

  \item[\acronym{TIF} format.]\itemend Here, \metavar{COMPRESSION} is
    one of the keywords:
    ...
    \item[deflate]\itemend Use the \propername{Deflate} compression
      scheme also called \acronym{ZIP}-in-\acronym{TIFF}.
      \propername{Deflate} is a lossless data compression algorithm
      that uses a combination of the \acronym{LZ77} algorithm and
      \propername{Huffman} coding.

      \genidx{compression!\acronym{JPEG} of \acronym{TIFF}}%
    \item[jpeg\optional{:\metavar{LEVEL}}]\itemend Use \acronym{JPEG}
      compression.  With optional argument set the
      compression~\metavar{LEVEL}, where \metavar{LEVEL} is an integer
      that ranges from 0--100.

      \genidx{compression!\acronym{LZW}}%
      \gensee{LZW@\acronym{LZW} compression}{compression}%
    \item[lzw]\itemend Use
      \propername{Lempel}-\propername{Ziv}-\propername{Welch}
      (\acronym{LZW}) adaptive compression scheme.  \acronym{LZW}
      compression is lossless.

      \genidx{compression!packbits}%
      \gensee{packbits compression}{compression}%
    \item[packbits]\itemend Use \propername{PackBits} compression
      scheme.  \propername{PackBits} is a particular variant of
      run-length compression; it is lossless.
    \end{codelist}

  \item[Any other format.]\itemend Other formats do not accept a
    \metavar{COMPRESSION} setting.  However, the underlying
    \uref{\hciiwrvigra}{\acronym{VIGRA}}\footnote{\acronym{VIGRA} is
      the image manipulaton library on which \App{} is based.} library
    automatically compresses \filename{png}-files with the
    \propername{Deflate} method.
  \end{description}
...
\item[--gpu \restrictednote{\acronym{OpenCL}-enabled versions
    only.}]\itemend Employ one of the graphics processing units
  (\acronym{GPU}s) to perform computing instead of the central
  processing units (\acronym{CPU}s) alone.  \App{} must have been
  compiled with support for \acronym{OpenCL} access to the
  \acronym{GPU} for this feature.

  Depending on the input images, the options passed to \App{}, and the
  relative performance of the \acronym{CPU}s to the \acronym{GPU}s
  this option may or may not increase performance.

  This option enables \acronym{GPU} processing on the selected
  \acronym{GPU}s.  To choose a particular \acronym{GPU} or override
  \App's default choice use
  \flexipageref{\sample{--prefer-gpu}}{opt:prefer-gpu}.

  Negate this option with
  \sample{--no-gpu}\optidx[\defininglocation]{--no-gpu}.
...
\item[\itempar{-l \metavar{LEVELS} \\ --levels=\metavar{LEVELS}}]\itemend
  Use at most this many \metavar{LEVELS}%
  \genidx{levels!pyramid}%
  \gensee{pyramid levels}{levels, pyramid}
  for pyramid\footnotemark{} blending if \metavar{LEVELS} is positive,
  or reduce the maximum number of levels used by $-\metavar{LEVELS}$
  if \metavar{LEVELS} is negative; \sample{auto} or \sample{automatic}
  restore the default, which is to use the maximum possible number of
  levels for each overlapping region.

  \footnotetext{As \propername{Dr.~Daniel Jackson}%
    \genidx{Jackson, Daniel@\propername{Jackson, Daniel}} correctly
    \ahref{\stargatewikiathetomb}{noted}, actually, it is not a
    pyramid: ``Ziggaurat, it's a
    \ahref{\wikipediaziggaurat}{Ziggaurat}.''}% FIXME
  ...
  Positive values of \metavar{LEVELS} limit the maximum number of
  ...
  of \metavar{LEVELS} reduce the number of pyramid levels below the
  maximum no matter what the actual maximum is and thus always
  influence all pyramids.  Use \sample{auto} or \sample{automatic} as
  \metavar{LEVELS} to restore the automatic calculation of the maximum
  number of levels.

  The valid range of the absolute value of \metavar{LEVELS} is
  \val{val:minimum-pyramid-levels} to
  \val{val:maximum-pyramid-levels}.
...
\item[\itempar{-o \metavar{FILE} \\ --output=\metavar{FILE}}]\itemend
  Place \appdoes{}ed output image in \metavar{FILE}.  If
  \sample{--output} is omitted, \App{} writes the resulting image to
...
\item[\itempar{-v \optional{\metavar{LEVEL}} \\ --verbose\optional{=\metavar{LEVEL}}}]\itemend
  ...
  Giving more \option{--verbose}~options will make \App{} more
  verbose; see Section~\fullref{sec:finding-out-details} for an
  examplary output.  Directly set a verbosity level with a
  non-negative integral~\metavar{LEVEL}.
  Table~\ref{tab:verbosity-levels} shows the messages available at a
  particular \metavar{LEVEL}.
  ...
  \begin{table}[htbp]
    ...
    \caption[Verbosity levels]{Verbosity levels of \app{}; each level
      includes all messages of the lower
      levels.\label{tab:verbosity-levels}}
  \end{table}
