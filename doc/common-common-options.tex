...
\item[\itempar{-o \metavar{FILE} \\ --output=\metavar{FILE}}]\itemend
  ...
  \begin{geeknote}
    We refrain from utilizing the \acronym{TIFF} as fallback output file type, because some of
    the libraries which implement \acronym{TIFF} ``seek'' the output image file.  Technically,
    this means they call fseek(3) or related functions, which would preclude, for example,
    \filename{/dev/stdout} to be used as \metavar{FILE}.
