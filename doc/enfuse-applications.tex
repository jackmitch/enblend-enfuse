...
\option{--saturation-weight=0.0} sometimes can be worth the extra run.
...
\App{}'s defaults, \code{exposure-weight}=\val{val:default-weight-exposure} and
\code{saturation-weight}=\val{val:default-weight-saturation} are well suited for fusion of
\emph{color} images.  Remember that saturation weighting only works for \acronym{RGB} data.
...
series.  Naively (ab)using \sample{--saturation-weight} will not work.
...
stack increases the depth-of-field (\acronym{DoF}) beyond the physical limits of diffraction.
...
\mbox{\code{\val{val:default-soft-mask-template}}}) and hard-mask files
...
contrast enhancement (\acronym{LCE}) helps.  Set \option{--contrast-edge-scale}=\feasiblebreak
\metavar{SCALE}:\feasiblebreak \metavar{LCE-SCALE}:\feasiblebreak \metavar{LCE-FACTOR}, where
\metavar{LCE-SCALE} and \metavar{LCE-FACTOR} work like the
\uref{\cambridgeincolourcomunsharpmask}{unsharp mask} filters in various image manipulation
programs.  Start with \metavar{LCE-SCALE} ten times the value of \metavar{SCALE} and a
\metavar{LCE-FACTOR} of 2--5.

\metavar{LCE-SCALE} can be specified as a percentage of \metavar{SCALE}.  \metavar{LCE-FACTOR}
...
measure~\metavar{CURVATURE}.  This forces all curvatures less than $-\metavar{CURVATURE}$ to
zero.
...
with a positive \sample{--contrast-min-curvature}.  A too high value of
\sample{--contrast-min-curvature} suppresses fine detail though.  Part of the detail can be
brought back with pre-sharpening, that is, \ref{sec:local-contrast-enhancement} or combining
\acronym{LoG} with local-contrast-window mode by using a negative
