...
\begin{sloppypar}
  ...
  \propername{Burt}\genidx{Burt@\propername{Burt, Peter J.}}-%
  \propername{Adelson}\genidx{Adelson@\propername{Adelson, Edward H.}}
  multiresolution spline\genidx{multiresolution spline}%
  \genidx{Burt-Adelson@\propername{Burt-Adelson} multiresolution
    spline} algorithm.\footnotemark{} This technique tries to make the
...
alpha\genidx{channel!alpha}\gensee{alpha channel}{channel, alpha}
...
\application{Hugin}\appidx{Hugin} or PanoTools\appidx{PanoTools}
to do this.  The \acronym{TIFF}~files produced by these programs are
exactly what \App{} is designed to work with.  Sometimes these
\acronym{GUI}s allow you to select feathering\genidx{feathering} for
...
\App{} blends the images in the order they are specified on the
