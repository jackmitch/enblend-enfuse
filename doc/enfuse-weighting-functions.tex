...
\flexipageref*{Overview}{sec:overview}, \App{} supports four different
...
the options~\option{--exposure-weight}, \option{--saturation-weight},
\option{--contrast-weight}, and \option{--entropy-weight}
...
Equal weights are another extreme that turns \eqnref{equ:weight} into
...
Use \sample{--hard-mask} to switch \App{} into a different weighting
...
option~\option{--hard-mask} \eqnref{equ:weight} becomes
...
noise-reduction property of the weighted average~\eqnref{equ:weight},
because only a single input pixel contributes to the output.


\subsection[Single Criterion Fusing]{Single Criterion Fusing
  \label{sec:single-criterion-fusing}
  \genidx{single criterion fusing}
  \genidx{fusing!single criterion}}

\App{} allows the user to weight each pixel of an input image by up to
four different criteria (see e.g.\ Chapter~\fullref{sec:overview}).
...
zero, which means they are \emph{active}.\genidx{criteria!active}%
...
with option~\option{--save-masks} and it will become clearer how
...
criterion completely overpower\genidx{verpowering
  criteria}\genidx{criteria!overpowering one another} all others.
