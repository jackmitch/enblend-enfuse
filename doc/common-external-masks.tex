...
In the usual workflow \application{Enblend} and \application{Enfuse}
...
and \option{--save-masks}.  See Chapter~\fullref{sec:invocation}, for
detailed explanations of both options.  With the help of these options
the processing can be broken up into two steps:

\begin{enumerate}
\item
  Save masks with \option{--save-masks}.  Generate masks and save them
  into image files.

  Avoid option~\option{--output} unless the blended or fused image at
  this point is necessary.

\item
  Load masks with \option{--load-masks}.  Load masks from files and
...
To further optimize this kind of workflow, both \application{Enblend}
and \application{Enfuse} stop after mask generation if
option~\option{--save-masks} is given, but \emph{no output file} is
specified with the \option{--output} option.  This way the time for
pyramid generation, blending, fusing, and writing the final image to
disk is saved, as well as no output image gets generated.

Note that options~\option{--save-masks} and \option{--load-masks}
