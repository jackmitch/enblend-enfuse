...
\item[--coarse-mask\optional{=\metavar{FACTOR}}]\itemend Use a
  scaled-down or ``coarse''\genidx{mask!coarse}\gensee{coarse mask}{mask, coarse}
  ...
  If omitted \metavar{FACTOR} defaults to
  \val{val:default-coarseness-factor}, this means,
  option~\option{--coarse-mask} shrinks the overlapping \emph{areas}
  ...
  Also see the negated option, \sample{--fine-mask} and
  ...
  \begin{table}[htbp]
    ...
    \caption[Mask generation options]{Various options that control the
      ...
      Nearest-Feature Transformation (\acronym{NFT}) of the overlap
...
\item[--load-masks\optional{=\metavar{IMAGE-TEMPLATE}}]\itemend
  Instead of generating masks, load\genidx{mask!load}\gensee{load
    mask}{mask, load} those in \metavar{IMAGE\hyp{}TEMPLATE}.  The
  default is \mbox{\sample{\val{val:default-mask-template}}}.  The
  mask images must be 8~bit grayscale images.

  See option~\option{--save-masks} below for details.


  \label{opt:optimize}%
  \optidx[\defininglocation]{--optimize}%
\item[--optimize]\itemend Use a multi-strategy approach to
  route\genidx{optimize!seam}\gensee{seam optimization}{optimize, seam}
  ...
  Option~\option{--no-optimize}\optidx[\defininglocation]{--no-optimize}
  negates \option{--optimize}%
  \genidx{optimize!seam}\gensee{seam optimization}{optimize, seam} and
  thus turns off seam line optimization.  Combined with
  option~\option{--fine-mask} this will produce the same type of mask
  as \App{}~version~2.5, namely the result of a Nearest-Feature
  Transform (\acronym{NFT}).\genidx{nearest-feature transform
    (\acronym{NFT})}
...
\item[--save-masks\optional{=\metavar{IMAGE-TEMPLATE}}]\itemend
  Save\genidx{mask!save}\gensee{save mask}{mask, save} the generated
  masks to \metavar{IMAGE-TEMPLATE}, which defaults to
  \mbox{\sample{\val{val:default-mask-template}}}.  \App{} saves masks
  ...
  edit to make your changes.  Later, use option~\option{--load-masks}
  to blend the project with your custom seam lines.

  \begin{sloppypar}
    \App{} will stop\genidx{save mask!only}\gensee{only save
      mask}{save mask, only} after saving all masks unless
  ...
  \begin{geeknote}
    \metavar{IMAGE-TEMPLATE} defines a template that is expanded for
    ...
    file (see Section~\fullref{sec:common-options}).
    ...
    file~\sample{\%D}, generates a two-digit index~\sample{\%02n} to
...
\item[--visualize\optional{=\metavar{VISUALIZE-TEMPLATE}}]\itemend
  Create an image according to \metavar{VISUALIZE-TEMPLATE} that
  visualizes%
  \genidx{mask!optimization visualization}%
  \gensee{visualization of mask}{mask, optimization visualization} the
  unoptimized mask and the applied optimizations (if any).  The
  default is \mbox{\sample{\val{val:default-visualize-template}}}.
  ...
  consult Chapter~\fullref{sec:visualization-image}.

  \begin{geeknote}
    \metavar{VISUALIZE-TEMPLATE} defines a template that is expanded
    for each input file.  In a template, a percent sign (\sample{\%})
    introduces a variable part; all other characters are copied
    literally.  Lowercase letters refer to the name of the respective
    input file, whereas uppercase ones refer to the name of the output
    file (see option~\flexipageref{\option{--output}}{opt:output}).
    Table~\fullref{tab:mask-template-characters} lists all variables.
  \end{geeknote}
\end{codelist}
