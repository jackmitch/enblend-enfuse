...
option~\option{--gpu}.  \App{} compiles, links and evaluates the exposure-weight function
identifed by \metavar{SYMBOL} from \metavar{OPENCL-SOURCE}, where \metavar{OPENCL-SOURCE} has
extension~\filename{.cl}:
...
\item
  ...
  \item[enfuse\_normalized\_luminance]\itemend
    Helper function which implements \equationabbr~\ref{equ:linear-luminance-transform}
    (\pageabbr~\pageref{equ:linear-luminance-transform}).
    \begin{cxxlisting}
...
float weight(float y)
{
  ...
  \caption[Generalized \propername{Gauss} weight function]{%
    ...
    definition of the default parameter~\code{EXPONENT}.}
\end{exemplar}
