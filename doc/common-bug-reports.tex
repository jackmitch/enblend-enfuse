...
end, visit the package's \genidx{bug!database at LaunchPad}\gensee{LaunchPad!bug database}{bug,
  database at LaunchPad}\uref{\launchpadnet}{LaunchPad} bug
...
\begin{itemize}
\item
  If \application{Enblend} or \application{Enfuse} get a fatal signal, for any options or input
  images, that is a bug.

\item
  If \application{Enblend} or \application{Enfuse} produce incorrect results, for any input
  whatever, that is a bug.

\item
  If \application{Enblend} or \application{Enfuse} produce an error message for valid input,
  that is a bug.

\item
  If \application{Enblend} or \application{Enfuse} do not produce an error message for invalid
  input, that is a bug.
\end{itemize}
...
\item
  The \emph{exact command line} you use to call \application{Enblend} or \application{Enfuse},
  ...
  If you call \application{Enblend} or \application{Enfuse} from within a \acronym{GUI} like,
  for example, \uref{\huginsourceforgenet}{Hugin}\appidx{Hugin} or
  \uref{\panoramadyndnsorgkimagefuser}{KImageFuser}\appidx{KImageFuser} by \propername{Harry van
    der Wolf}, copy\&paste or write down the command line that launches \application{Enblend} or
  \application{Enfuse}.
...
\section[Sending Patches]{\label{sec:sending-patches}%
  ...
  Sending Patches for \application{Enblend} or \application{Enfuse}}

If you would like to write bug fixes or improvements for \application{Enblend} or
\application{Enfuse}, that is very helpful.  When you send your changes, please follow these
guidelines to avoid causing extra work for us in studying the patches.  If you do not follow
these guidelines, your information might still be useful, but using it will take extra work.
