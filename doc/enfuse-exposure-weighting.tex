...
\section[Exposure Weighting]{Exposure Weighting
  ...
  \genidx{exposure weighting}
  \genidx{weighting!exposure}}

Exposure weighting prefers pixels with a luminance~$Y$ close to the
user-chosen optimum value
(option~\option{--exposure-optimum},\optidx{--exposure-optimum}
default: \val{val:default-exposure-optimum}) of the normalized,
real-valued luminance interval~$(0, 1)$.

\acronym{RGB}-pixels are converted to luminance before using the
grayscale projector\genidx{grayscale
  projector}\genidx{projector!grayscale} given by
\sample{--gray-projector},\optidx{--gray-projector} which defaults
to \code{average}.  Grayscale pixels are identified with luminance.

In the normalized luminance interval\genidx{normalized luminance
  interval}\genidx{luminance interval!normalized} 0.0 represents pure
black and 1.0 represents pure white independently of the data type of
the input image.  This is, for a \acronym{JPEG} image the
luminance~255 maps to 1.0 in the normalized interval and for a 32~bit
\acronym{TIFF} picture the highest luminance value~4294967295 also
...
\propername{Gaussian}\genidx{exposure weight
  function!\code{gauss}}\genidx{exposure weight
  function!\propername{Gaussian}}
\[
    w_{\mathrm{exp}}(Y) =
    \exp\left(-\frac{1}{2}
...
controlled by the command line options \option{--exposure-optimum}
and~\option{--exposure-width} respectively, where $Y_{\mathrm{opt}}$
defaults to \val{val:default-exposure-optimum} and $width$ defaults to
\val{val:default-exposure-width}.  Figure~\ref{fig:gaussian} shows
some \propername{Gaussians}.
...
\begin{figure}[htbp]
  ...
  \caption[\propername{Gaussian} weight function]{\App{}'s
    \propername{Gaussian} function with the parameters
    \metavar{optimum} = 0.5 and three different \metavar{width}s: 0.1,
    0.2, and~0.4.\label{fig:gaussian}}
\end{figure}


The options \option{--exposure-optimum}\optidx{--exposure-optimum}
and~\option{--exposure-width}\optidx{--exposure-width} serve to
fine-tune the final result without changing the set of input images.
Option~\option{--exposure-optimum} sets the point of optimum exposure.
Increasing the \metavar{optimum} makes \App{} prefer lighter pixels,
rendering the final image lighter, and vice versa.
Option~\option{--exposure-width} defines the \metavar{width} of
acceptable exposures.  Small values of \metavar{width} penalize
exposures that deviate from \metavar{optimum} more, and vice versa.

In \App{} version~4.2 several new exposure weight functions have been
added.  Select them with
option~\option{--exposure-weight-function}.\optidx{--exposure-weight-function}
...
\propername{Gaussian} and \propername{Lorentzian} behave like $1 -
z^2$ around the optimum.  However for large $|z|$ the
\propername{Gaussian} weight rolls off like $\exp(-z^2/2)$ and the
\propername{Lorentzian} only as $z^{-2}$.
...
\begin{figure}[htbp]
  ...
  \caption[\propername{Lorentzian} function]{\App{}'s
    \propername{Lorentzian} function with the parameters
    $\metavar{optimum} = 0.5$ and three different \metavar{width}s:
    0.1, 0.2, and~0.4.\label{fig:lorentzian}}
\end{figure}


Both, the \propername{Gaussian} and the \propername{Lorentzian} are
...
\begin{figure}[htbp]
  ...
  \caption[Half-Sine function]{\App{}'s Half-Sine function with the
    ...
    \metavar{width}s: 0.1, 0.2, and~0.4.\label{fig:halfsine}}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \caption[Full-Sine function]{\App{}'s Full-Sine function with the
    parameters $\metavar{optimum} = 0.5$ and three different
    \metavar{width}s: 0.1, 0.2, and~0.4.\label{fig:fullsine}}
\end{figure}


Again, Half-Sine and Full-Sine behave like $1 - z^2$ around the
optimum, like \propername{Gaussian} and \propername{Lorentzian}.
...
\begin{figure}[htbp]
  ...
  \caption[Bi-Square function]{\App{}'s Bi-Square function with the
    parameters $\metavar{optimum} = 0.5$ and three different
    \metavar{width}s: 0.1, 0.2, and~0.4.\label{fig:power}}
\end{figure}
...
result.  However, if the \metavar{width} is chosen so large that the
...
option~\option{--exposure-cutoff},\optidx{--exposure-cutoff} which
...
\begin{figure}[htbp]
  ...
  \caption[Comparison of exposure weight functions]{Comparison of all
    ...
    share the same \acronym{FWHM}.\label{fig:exposure-weights}}
\end{figure}
...
\metavar{optimum} or \metavar{width}, making it simple to just try
...
On operating systems, where dynamic loading\genidx{dynamic
  loading}\genidx{loading!dynamic} of code is possible and for \App{}
executables compiled with dynamic-linking support (see
Section~\fullref{sec:finding-out-details} on how to check this
feature), \App{} can work with user-defined exposure weighting
functions, passed with the long form of
option~\option{--exposure-weight-function}.  It loads the exposure
weight function identifed by \metavar{SYMBOL} from a
\metavar{SHARED-OBJECT}:\genidx{shared object}\genidx{object!shared}%
\genidx{dynamic library}\genidx{library!dynamic}
\begin{literal}
...
\item
  ...
  \metavar{SHARED-OBJECT} are suitable for \metavar{SYMBOL} without
  knowledge of source code of \metavar{SHARED-OBJECT}.
\end{itemize}
...
\item
  ...
  support''.\genidx{dynamic linking support}
...
\item
  ...
  \item
    ...
    that compiled \App{}.
  \end{enumerate}
  The latter is called \acronym{ABI}-compatible.  An example for a
  pair of \acronym{ABI}-compatible compilers is
  \acronym{GNU}'s~\command{g++} and
...
\item
  The base-class header file \filename{exposure\_weight\_base.h} is
  available.
\end{enumerate}
...
\item
  \begin{sloppypar}
    ...
    base-class~\code{ExposureWeight}, which is defined in header
    file~\filename{exposure\_weight\_base.h}.  It resides in the
    \filename{src}~sub-directory of the source distribution and -- for
    a correctly installed package -- in directory
    \filename{\val*{val:DATAROOTDIR}/\feasiblebreak
...
\item
  ...
  \begin{itemize}
  \item
    Domain: define \code{weight} for normalized luminance
    values~\metavar{y} from zero to one including both ends.

  \item
    Image: Let the \code{weight}s fall in the interval from zero to
    one.
  ...
  \item
    (Optionally) Rescale the \metavar{WIDTH} of the function to match
    the \acronym{FWHM} of \App{}'s original Gauss curve.  The
    macro~\code{FWHM\_GAUSSIAN} is defined exactly to this end.
  \end{itemize}

\item
  If necessary, rewrite methods~\code{initialize} and
  \code{normalize}, too.

\item
  \App{} never calls \code{initialize} in an \acronym{OpenMP} parallel
  execution environment.  However, \acronym{OpenMP}-enabled versions
  of \App{} call \code{normalize} and \code{weight} in parallel
  sections.

  Technically, the functors which the user-defined weight functions are
  part of are copy-constructed for each \acronym{OpenMP} worker thread.
  Thus, there is no contention within the \code{ExposureWeight}
  sub-classes.  Although, if \code{normalize} or \code{weight} access a
  shared resource these accesses must be protected by serialization
  instructions.  One solution is to use \acronym{OpenMP} directives,
  like for example,
...
#pragma omp critical
{
  ...
  constructs, like, for example \code{\#pragma omp atomic} or simply
  an atomic data-type (e.g.~\code{sig\_atomic\_t} from
  \filename{signal.h}).

  Remember to compile all modules that use \acronym{OpenMP} directives
  with the (compiler-specific) flags that turn on \acronym{OpenMP}.
  For \command{g++} this is \sample{-fopenmp} and for \command{icpc}
  it is \sample{-fopenmp} or \sample{-openmp}.

\item
  To raise an exception associated with the derived, user-defined
  exposure-weight class, throw \code{ExposureWeight::error}.  \App{}
  catches these exceptions at an enclosing scope, displays
  \metavar{a\_message}, and then aborts.

\item
  Define an object of the derived class.  This creates the
  \metavar{SYMBOL} to refer to at the \App{} command line.

  The actual signature of the constructor (default, two-argument,
  \dots) does not matter, because \App{} \emph{always} invokes
  \code{initialize} before calling any other method of a user-defined
  \code{ExposureWeight} sub-class.  Method~\code{initialize} sets
  (read: overwrites) \metavar{optimum} and \metavar{width} and ensures
...
C++-code of a suitable extension\footnote{If \App{} has been compiled
  ...
  directory~\filename{\val*{val:DATAROOTDIR}/enfuse/xweight} along
  with a \acronym{GNU}-Makefile called
  \filename{Makefile.userweight}.}.
...
Linear linear;
  ...
  \caption[Simple dynamic exposure weight function]{A dynamic exposure
    ...
    exactly one, so we override method~\code{initialize} to rescale
    \metavar{WIDTH}, passed in as \code{width\_parameter}, by
    multiplying with \code{FWHM\_GAUSSIAN} to get the same width as
...
TemplatedPower<4> tpower4;
  ...
  \caption[Templated dynamic exposure weight function]{The templated
    class~\code{TemplatedPower} allows to create a weight function for
    arbitrary positive exponents~\code{n}.  In particular,
    \code{TemplatedPower<4>} duplicates the built-in exposure-weight
    function~\code{bisquare}.\label{ex:templated-dynamic-exposure-weight-function}}
\end{exemplar}


The last example, \ref{ex:variable-dynamic-exposure-weight-function},
shows a weight function that accesses an extra \metavar{ARGUMENT}
passed in with \option{--exposure-weight-function}.  A class like
\code{VariablePower} allows full control over the exponent at the
...
VariablePower vpower;
  ...
  \caption[Dynamic exposure weight function with extra
    ...
    option~\option{--exposure-weight-function}.\label{ex:variable-dynamic-exposure-weight-function}}
\end{exemplar}
...
Exposure weighting objects are created and destroyed only
$O(1)$~times.  Thus, method~\code{initialize} could be used to perform
all kinds of computationally expensive tasks.  However,
methods~\code{normalize} and \code{weight} are called for every pixel
...
\begin{restrictedmaterial}{Linux}
  \noindent Compile and link using the \acronym{GNU} compiler for
  example with
  \begin{literal}
    g++ -std=c++11 \bslash \\
  ...
  \item[\option{-fpic}]\itemend Instruct the compiler's code-generator
    to produce position\hyp{}independent code (\acronym{PIC}), which
    is suitable for a shared object.  Some systems require
    \sample{-fPIC} instead of \sample{-fpic}.
  ...
  \metavar{SYMBOL}~name in the \metavar{SHARED-OBJECT}.  The function
  has access to additional \metavar{ARGUMENT}s passed in with the
...
\begin{restrictedmaterial}{Windows}
  ...
  link libraries (\acronym{DLL}~files) as they are called here -- has
  been tested with the \acronym{MinGW} compiler chain and with
  MS-Visual~C++~2012.
  ...
  \item
    ...
    \end{literal}
    For details see the explanation for the \acronym{GNU} compiler
    above.  Windows neither requires options \option{-fpic}
    nor~\option{-fPIC}.

  \item
    When using the MS-Visual~C++~compiler, you need to explicitly
    export \metavar{SYMBOL}.  There are two possiblities to achieve
    this.  Use only one variant, not both at the same time.

    \begin{enumerate}
    \item
      Either use \code{"C"}~linkage and define the object using the
      construction \code{\_\_declspec(dllexport)}.  For
...
extern "C"
{
  __declspec(dllexport) Linear linear;
    ...
    \item
      ...
      (\filename{.def}) and pass this file to the linker (in:
      \guielement{Project Properties}, \guielement{Linker},
      \guielement{Module Definition File}).  For
  ...
  Finally, the weight function can be selected by its \metavar{SYMBOL}
