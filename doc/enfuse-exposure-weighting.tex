...
\acronym{RGB}-pixels are converted to luminance before using the grayscale projector given by
\sample{--gray-projector}, which defaults to \code{average}.  Grayscale pixels are identified
with luminance.
...
\propername{Gaussian} and \propername{Lorentzian} behave like $1 - z^2$ around the optimum.
However for large $|z|$ the \propername{Gaussian} weight rolls off like $\exp(-z^2/2)$ and the
...
not go exactly to zero.  Thus, \App{} can select ``better'' pixels far away from the chosen
optimum.
...
Again, Half-Sine and Full-Sine behave like $1 - z^2$ around the optimum, like
\propername{Gaussian} and \propername{Lorentzian}.  However for large $|z|$ they both are
exactly zero.  The difference is how they decrease just before they reach zero.  Half-Sine
behaves like $z - z'$ and Full-Sine like $(z - z'')^2$, where $z'$ and $z''$ are the respective
zeros.
...
Bi-Square is the only predefined function that behaves like $1 - z^4$ around the optimum.
...
\begin{figure}[htbp]
  ...
  \caption[Comparison of exposure weight functions]{\label{fig:exposure-weights}%
    Comparison of all of \App{}'s exposure weight functions for the default values of
    $\metavar{optimum} = 0.5$ and $\metavar{width} = 0.2$.  Note that all functions intersect at
    $w = \slfrac{1}{2}$, this is, they share the same \acronym{FWHM}.}
\end{figure}
...
\item
  ...
  \code{ExposureWeight::error}.  \App{} catches these exceptions at an enclosing scope, displays
...
However, methods~\code{normalize} and \code{weight} are called for every pixel in each of the
input images.  Therefore, if performance of the weight function is a problem, these two
functions are the prime candidates for optimization.
...
\begin{restrictedmaterial}{Linux}
  ...
  \item[\option{-shared}]\itemend
    ...
    systems, the library must be blessed, buy passing an shared-object name (\option{soname})
