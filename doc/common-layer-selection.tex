...
Some image formats, like for example \acronym{TIFF}, allow for storing
...
\emph{ordered} set of images.  In the \acronym{TIFF}-documentation
these are known as
``multi-page''\genidx[\defininglocation]{file!multi-page}%
\gensee{multi-page file}{file, multi-page}\gensea{multi-page
  file}{layer} files and because the image data in a
\acronym{TIFF}-file is associated with a
``directory'',\genidx[\defininglocation]{image!directory} the files
sometimes are also called ``multi-directory'' files.  In this manual,
multiple images in a file are called
``layers''.\genidx[\defininglocation]{layer!image}
...
\filename{taoth-vaclarush} or \filename{valos-cor}, but only numbers.
...
\metavar{layer-specification} to the image file name.
Table~\ref{tab:layer-selection-grammar} defines the grammar of
\metavar{layer-specification}\/s.

Selecting a tuple of layers with a \metavar{layer-specification}
overrides the active layer selection algorithm.  See also
option~\flexipageref{\option{--layer-selector}}{opt:layer-selector}
and Section~\fullref{sec:response-files}.
...
\begin{table}[htbp]
  ...
  \caption[Grammar of layer specifications]{\acronym{EBNF} definition
    of the grammar of layer specifications.  In addition to the
    \metavar{selection} separator~\sample{:} shown all usual
    numeric-option delimiters
    (\sample{\val{val:numeric-option-delimiters}}) apply.  The keyword
    for \metavar{range} reversal,
    \sample{\val{val:layer-range-reverse-keyword}}, can be abbreviated
...
The simplest \metavar{layer-specification} are the
layer-\metavar{index}es themselves.  The first layer gets index~1, the
...
The single layer of a single-layer file always can be accessed either
with index~\sample{1} or \sample{-1}.

Select a contiguous \metavar{range} of indexes with the range
operator~\sample{\val{val:layer-range-separator}}, where the
\metavar{range-bound}\/s are forward or backward indices.  Leaving out
a bound or substituting the open-range
indicator~\sample{\val{val:layer-range-empty-index-symbol}} means a
...
\end{terminal}
works, whereas spaced-out out phrase \code{multi-layer.tif [2 : ]}
must be quoted
\begin{terminal}
  \$ \app{} --output=out.tif --verbose 'multi-layer.tif[2 : ]'
\end{terminal}
...
\item[\lspec{}] The empty selection selects nothing and in that way
  works like the layer-selector \sample{no-layer}.
...
\begin{geeknote}
  ...
  to an error with \application{Enblend}, but may be desired with
  \application{Enfuse} in \code{soft-mask}~mode to give the image more
...
\begin{itemize}
\item
  \application{Hugin}'s stitcher, \command{nona} produces
  multi-page \acronym{TIFF}~file, when called with the \sample{-m
    TIFF\_multilayer}-option.

\item
  The utility \command{tiffcp} of the
  \acronym{TIFF}-\uref{\remotesensingorglibtiff}{LibTIFF tool suite}
  merges several \acronym{TIFF}-images into a single multi-page
  file.

\item
  The sister program \command{tiffsplit} splits a multi-page file
  into a set of single-page \acronym{TIFF}-images.

\item
  Another utility of the same origin, \command{tiffinfo}, is very
  helpful when inquiring the contents of single- or multi-page
  file \acronym{TIFF}-files.

\item
  All tools of the \uref{\imagemagickorg}{ImageMagick} suite, like,
  for example, \command{convert} and \command{display} use a
  \uref{\imagemagickorgcommandlineprocessinginput}{similar syntax} as
  \App{} to select layers (which in ImageMagick parlance are called
  ``frames''\genidx{image!frame}) in multi-page files.  Please
  note that ImageMagick tools start indexing at zero, whereas we start
  at one.

\item
  \App{} and \OtherApp{} by default apply the
  \sample{\val{val:layer-selector}} selector (see
  option~\flexipageref{\option{--layer-selector}}{opt:layer-selector})
...
Appendix~\fullref{sec:helpful-programs}.
