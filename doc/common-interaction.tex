...
\noindent The information are explained in detail in the following sections.
...
The twelve-digit hexadeximal \metavar{ID-CODE} is automatically generated by our source-code
...
\begin{exemplar}
  ...
  \caption[Questions answered by \sample{\app{} --version --verbose}]%
          {\label{ex:qanda-option-version-verbose}%
            Some questions that calling \appcmd{} with the option pair \option*{--version}
            and~\option*{--verbose} can answer.}
\end{exemplar}
...
do not match the ones it was compiled with.  Option~\option{--show-software-components} can be
